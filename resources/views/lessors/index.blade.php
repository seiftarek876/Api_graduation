        ...
        <div class="max-w-full mx-auto sm:px-3 lg:px-4">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class="flex justify-center m-3">
                    <x-button-link>
                        Add New Lessor
                        <x-slot name="href">{{route('lessors.create')}}</x-slot>
                    </x-button-link>
                </div>
                {{-- {{dd($bins)}} --}}
                    ...
                    <div class="overflow-x-auto sm:mx-0.5 lg:mx-0.5">
                        <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
                            <div class="overflow-hidden">
                                <table class="min-w-full">
                                    ...
                                    @forelse($lessors as $key=> $sub)
                                        <tr class="bg-white border-b text-center transition duration-300 ease-in-out hover:bg-gray-100">
                                            ...
                                            <td class="text-sm text-gray-900 font-bold px-6 py-4 whitespace-nowrap">
                                                {{$sub->subscribtion_fee}}
                                            </td>
                                            <td class="text-sm text-gray-900 font-bold px-6 py-4 whitespace-nowrap">
                                                {{date_format(date_create($sub->created_at),'Y-m-d h:i:s a')}}
                                            </td>
                                            <td>
                                                <div class="flex justify-evenly">
                                                    <div>
                                                        <a href="{{route('lessors.edit',$sub->id)}}"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 50 50">
                                                    ...
                                                    <div>
                                                        <form method="post"
                                                              action="{{route('lessors.destroy',$sub->id)}}">
                                                            @method('DELETE')
                                                            @csrf
                                                            <button type="submit"><svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="30" height="30" viewBox="0 0 30 30">
                                ...
                                <div class="mt-2">{{$lessors->links()}}</div>
                            </div>
                        </div>
                    </div>
                </div>
